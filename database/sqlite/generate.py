...
con = sqlite3.connect("../profs.sqlite")
cur = con.cursor()
...
def main():
    ...
    for email in faculty[:NUM_ORG]:
        ...
        cur.execute("INSERT INTO organizer VALUES (?, ?, ?)", (YEAR, email, roles))
    con.commit()
    ...
    for i in range(NUM_WORK):
        ...
        cur.execute("INSERT INTO person_workshop VALUES (?, ?, 'Lead')", (email, i+1))
    for i in range(NUM_WORK):
        email = students[2*i]
        cur.execute("INSERT INTO person_workshop VALUES (?, ?, 'Assist')", (email, i+1))
    ...
    for i in range(NUM_WORK):
        ...
        for slot in range(beg, end+1):
            cur.execute("INSERT INTO workshop_timeslot VALUES (?, ?, ?)", (i+1, YEAR, slot))
    con.commit()
